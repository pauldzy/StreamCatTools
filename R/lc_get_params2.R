lc_get_params2 <- function(param = NULL) {
  ...
  DSNAME <- NULL
  result <- tryCatch({
    resp <- jsonlite::fromJSON("https://api.epa.gov/StreamCat/lakes2/metrics")$items
    if (param=='aoi'){
      ...
      params <- httr2::request('https://api.epa.gov/StreamCat/lakes2/variable_info') |>
        httr2::req_perform() |>
...
lc_fullname2 <- function(metric = NULL) {
  result <- tryCatch({
    resp <-jsonlite::fromJSON("https://api.epa.gov/StreamCat/lakes2/datadictionary")$items
...
lc_get_metric_names2 <- function(category = NULL,
                                aoi = NULL,
                                year = NULL,
                                dataset = NULL) {
  if (!is.null(aoi)){
