...
sc_get_params2 <- function(param = NULL) {
  ...
  DSNAME <- NULL
  result <- tryCatch({
    resp <- jsonlite::fromJSON("https://api.epa.gov/StreamCat/streams2/metrics")$items
    if (param=='aoi'){
...
sc_fullname2 <- function(metric = NULL) {
  result <- tryCatch({
    resp <- jsonlite::fromJSON("https://api.epa.gov/StreamCat/streams2/datadictionary")$items
...
sc_get_metric_names2 <- function(category = NULL,
                                aoi = NULL,
                                year = NULL,
                                dataset = NULL) {
  if (!is.null(aoi)){
